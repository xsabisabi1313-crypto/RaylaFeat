...
TArray<FIntPoint> AUnit::GetAvailableMovePoss()
{
	...
	case EMovePatterns::Cross:
	{
		...
		TArray<FIntPoint> Candidates = {
			FIntPoint(CurrentPos.X + 1, CurrentPos.Y), // 右
			FIntPoint(CurrentPos.X - 1, CurrentPos.Y), // 左
			FIntPoint(CurrentPos.X,     CurrentPos.Y + 1), // 上
			FIntPoint(CurrentPos.X,     CurrentPos.Y - 1)  // 下
		};

		// 範囲内（X: 0〜2, Y: 1〜5）のものだけを厳選して追加する
		for (const FIntPoint& Pos : Candidates)
		{
			// Xが 0未満( < 0 ) または 3以上( >= 3 ) なら除外
			// Yが 0以下( <= 0 ) または 6以上( >= 6 ) なら除外
